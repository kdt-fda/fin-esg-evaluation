...
export default function App() {
  ...
  // 다음날 0시까지 남은 시간 계산 (1분마다)
  useEffect(() => {
    const updateTimer = () => {
      const now = new Date();

      const nextDate = formatYYYYMMDD(now);
    ...
    <div className="flex h-screen bg-gray-50">
      {selectedStock ? (
        ...
        <div className="w-80 bg-white border-r border-gray-200 h-screen flex flex-col">
          ...
          <div className="p-6 text-sm text-gray-500">
            {errorMsg ? <span className="text-red-600">{errorMsg}</span> : '잠시만 기다려주세요.'}
          </div>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-200 px-8 py-6">
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600">
              현재 종목:{' '}
              <span className="font-semibold text-blue-600">{selectedStock?.name ?? '-'}</span> (
              {selectedStock?.code ?? '-'})
            </p>
          </div>
        </header>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-6">
                ...
                <div className="text-sm text-gray-500 italic">* 차트의 예측 구간에 마우스를 올려 예측 근거를 확인하세요</div>
              </div>
            </div>
          </div>
        ...
        <footer className="bg-white border-t border-gray-200 px-8 py-4">
          <div className="text-center">
            <p className="text-gray-700 mb-1">{formatYYYYMMDD(new Date(Date.now() - 86400000))} 일까지의 데이터 반영했습니다.</p>
